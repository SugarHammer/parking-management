...
<div class="div_container">
	<div class="searchDiv">
		<form action="<%=basePath%>RoleHandle" method="get">
			<span>查询条件：</span>
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				out.print("<tr><td>"+obj[0]+"</td><td>"+obj[1]+"</td><td><a href='"+path+"/role/RoleEdit.jsp?role_id="+obj[0]+"' class='a_edit'>编辑</a><a href='"+path+"/RoleHandle?type=1&role_id="+obj[0]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a></td></tr>");
	                		}
                		}
